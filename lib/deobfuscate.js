...
export function decsojson4(jsf) {
...
export function obdec_default(jsf, thr1 = false, throwederror = '') {
  const ojs = jsf;
  let spljs;
  try {
    function sandbox() {
...
export function dec_sojsonv5_default(jsf) {
...
export function dec_jsjiamiv6_default(jsf) {
  const js = obdec_default(jsf);
  return js.substring(0, js.lastIndexOf(';'));
}

export function dec_list(jsf) {
  if (
    !(
      jsf.indexOf(',') != -1 &&
      jsf.indexOf('=') != -1 &&
...
export function decsojsonp(jsf) {
  if (jsf.indexOf("var __encode ='sojson.com") == -1) {
